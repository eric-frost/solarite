...
const terserOptions = {
	...
	compress: { // https://github.com/terser/terser#compress-options
		...
		// Comma-chaining saves minified bytes but costs compressed bytes. The commas
		// produce more distinct byte sequences than repeated '};' lines.
		sequences: false,
	},
...
/**
 * Write a JSX runtime and a JSX dev runtime beside each bundle, as thin modules that import that bundle.
 *
 ...
 *
 * The same goes for which bundle.  Someone who imports Solarite.min.js by path, as every example in
 * the documentation does, and maps "solarite/jsx-runtime" to a runtime that imports Solarite.js gets
 * two copies again.  So each bundle has a runtime of the same name: Solarite.js with jsx-runtime.js,
 * and Solarite.min.js with jsx-runtime.min.js.
 */
async function writeJsxRuntime() {
	let dir = output.replace(/[^/\\]+$/, '');
	let header = '// Generated by build/build.js from src/jsx-runtime.js — do not edit.\n';

	for (let bundle of [output, outputMin]) {
		let name = bundle.replace(/^.*[/\\]/, '');
		let suffix = name.replace(/^[^.]+/, ''); // '.js' or '.min.js'

		// Take the source verbatim and repoint its imports at the bundle. Copying it
		// instead of re-exporting it keeps these six small functions out of the main
		// bundle, so a project that never writes JSX does not download them. Rewriting
		// instead of hand-copying keeps src/jsx-runtime.js as the single source of truth.
		...
		// The minified runtime is minified WITHOUT property mangling.  It is built apart from the bundle, so
		// the two could not agree on mangled names; it only touches names the bundle is told to keep (name,
		// value, key, children), which is what makes pairing it with Solarite.min.js safe at all.
		if (suffix === '.min.js')
			src = (await Terser.minify(src, {ecma: 8, module: true, format: {ascii_only: true}})).code + '\n';

		Deno.writeTextFileSync(dir + 'jsx-runtime' + suffix, header + src);
		Deno.writeTextFileSync(dir + 'jsx-dev-runtime' + suffix, header +
			`export {jsx, jsxs, jsxDEV, Fragment, jsxTemplate, jsxAttr, jsxEscape} from './jsx-runtime${suffix}';\n` +
			`export {jsxDEV as jsxDEVRuntime} from './jsx-runtime${suffix}';\n`);
	}
}
